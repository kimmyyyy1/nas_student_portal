                   ...
                   class="bg-white overflow-hidden shadow-sm rounded-lg p-4 border-t-4 border-blue-600 flex flex-col justify-between hover:shadow-md transition cursor-pointer transform hover:-translate-y-1 {{ !request('status') ? 'ring-2 ring-blue-500 bg-blue-50' : '' }}">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-xl md:text-2xl font-extrabold text-gray-800" id="stat-submitted">{{ $totalSubmitted ?? 0 }}</p>
                        </div>
                   ...
                   class="bg-white overflow-hidden shadow-sm rounded-lg p-4 border-t-4 border-yellow-500 flex flex-col justify-between hover:shadow-md transition cursor-pointer transform hover:-translate-y-1 {{ request('status') == 'Pending' ? 'ring-2 ring-yellow-500 bg-yellow-50' : '' }}">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-xl md:text-2xl font-extrabold text-gray-800" id="stat-pending">{{ $countPending ?? 0 }}</p>
                        </div>
                        <i class='bx bx-time text-2xl text-yellow-200'></i>
                    </div>
                    <p class="text-[10px] md:text-xs text-yellow-600 mt-2 font-medium">Pending Review</p>
                </a>
                   ...
                   class="bg-white overflow-hidden shadow-sm rounded-lg p-4 border-t-4 border-green-500 flex flex-col justify-between hover:shadow-md transition cursor-pointer transform hover:-translate-y-1 {{ request('status') == 'Qualified' ? 'ring-2 ring-green-500 bg-green-50' : '' }}">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-xl md:text-2xl font-extrabold text-gray-800" id="stat-qualified">{{ $countQualified ?? 0 }}</p>
                        </div>
                        <i class='bx bx-check-circle text-2xl text-green-200'></i>
                    </div>
                    <p class="text-[10px] md:text-xs text-green-600 mt-2 font-medium">Passed</p>
                </a>
                   ...
                   class="bg-white overflow-hidden shadow-sm rounded-lg p-4 border-t-4 border-indigo-500 flex flex-col justify-between hover:shadow-md transition cursor-pointer transform hover:-translate-y-1 {{ request('status') == 'Waitlisted' ? 'ring-2 ring-indigo-500 bg-indigo-50' : '' }}">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-xl md:text-2xl font-extrabold text-gray-800" id="stat-waitlisted">{{ $countWaitlisted ?? 0 }}</p>
                        </div>
                        <i class='bx bx-list-ul text-2xl text-indigo-200'></i>
                    </div>
                    <p class="text-[10px] md:text-xs text-indigo-600 mt-2 font-medium">On Hold</p>
                </a>
                   ...
                   class="bg-white overflow-hidden shadow-sm rounded-lg p-4 border-t-4 border-red-500 flex flex-col justify-between hover:shadow-md transition cursor-pointer transform hover:-translate-y-1 {{ request('status') == 'Not Qualified' ? 'ring-2 ring-red-500 bg-red-50' : '' }}">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-xl md:text-2xl font-extrabold text-gray-800" id="stat-rejected">{{ $countRejected ?? 0 }}</p>
                        </div>
                ...
                <div class="p-4 md:p-6 text-gray-900">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                        <div class="flex items-center w-full md:w-auto justify-between md:justify-start">
                            ...
                            @if(request('status') || request('search'))
                                <a href="{{ route('admission.index') }}" wire:navigate class="ml-4 text-xs text-red-500 hover:underline flex items-center md:hidden">
                        ...
                        <form method="GET" action="{{ route('admission.index') }}" class="flex w-full md:w-auto">
                            ...
                            <div class="relative w-full md:w-64">
                                ...
                                <input type="text" name="search" placeholder="Search Name/LRN..." value="{{ request('search') }}" 
                    ...
                    @if(request('status') || request('search'))
                        <div class="hidden md:block mb-4">
                             <a href="{{ route('admission.index') }}" wire:navigate class="text-xs text-red-500 hover:underline flex items-center">
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 md:px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Details</th>
                                    <th class="hidden md:table-cell px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Applied For</th>
                                    <th class="hidden md:table-cell px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Dates</th>
                                    <th class="px-4 md:px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Status</th>
                                    <th class="px-4 md:px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Action</th>
                                </tr>
                            </thead>
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-4 md:px-6 py-4">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-3">
                                                ...
                                                <div class="text-[10px] md:text-xs text-gray-500">App ID: #{{ str_pad($app->id, 4, '0', STR_PAD_LEFT) }}</div>
                                                
                                                {{-- Mobile View Details --}}
                                                <div class="md:hidden mt-1 text-[10px] text-gray-500">
                                                    <span class="bg-gray-100 px-1 rounded">{{ $app->grade_level_applied }}</span> • 
                                    ...
                                    <td class="hidden md:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <div class="flex flex-col text-xs">
                                            ...
                                            @if($app->date_checked)
                                                <span class="text-green-600">Checked: {{ \Carbon\Carbon::parse($app->date_checked)->format('M d, Y') }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                        @if(in_array($app->status, ['Pending', 'pending', 'For Assessment']))
                                            <span class="px-2 inline-flex text-[10px] md:text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800 border border-yellow-200">
                                                Assessment
                                            </span>
                                        @elseif(in_array($app->status, ['Qualified', 'qualified']))
                                            <span class="px-2 inline-flex text-[10px] md:text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 border border-green-200">
                                                Qualified
                                            </span>
                                        @elseif(in_array($app->status, ['Waitlisted', 'waitlisted']))
                                            <span class="px-2 inline-flex text-[10px] md:text-xs leading-5 font-semibold rounded-full bg-indigo-100 text-indigo-800 border border-indigo-200">
                                                Waitlisted
                                            </span>
                                        @elseif(in_array($app->status, ['Not Qualified', 'not qualified']))
                                            <span class="px-2 inline-flex text-[10px] md:text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 border border-red-200">
                                                Failed
                                            </span>
                                        @else
                                            <span class="px-2 inline-flex text-[10px] md:text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">
                                                {{ $app->status }}
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 md:px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <a href="{{ route('admission.show', $app->id) }}" wire:navigate class="text-indigo-600 hover:text-white border border-indigo-600 hover:bg-indigo-600 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-xs px-3 py-1.5 text-center inline-flex items-center transition">
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            @if(request('search') || request('status'))
                                <a href="{{ route('admission.index') }}" wire:navigate class="text-blue-600 hover:underline text-sm mt-2 block">Clear Filters</a>
                            @endif
                        </div>
                    @endif
