<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('staff.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow-sm text-sm">
                Add Staff
            </a>
        </div>
    </x-slot>
                    ...
                    <div class="overflow-x-auto"> {{-- Added overflow wrapper --}}
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Full Name</th>
                                ...
                                @forelse ($staff as $person)
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            ...
                                            <a href="{{ route('staff.edit', $person->id) }}" class="text-indigo-600 hover:text-indigo-900 font-bold mr-3">Edit</a>
                                            
                                            <form class="inline-block" method="POST" action="{{ route('staff.destroy', $person->id) }}" onsubmit="return confirm('Sigurado ka bang gusto mong burahin ang staff na ito?');">
                                                @csrf
                                                @method('DELETE')
                                    ...
                                    <tr>
                                        <td colspan="4" class="px-6 py-10 whitespace-nowrap text-center text-gray-500">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <p class="text-sm">Click "Add Staff" to add a new record.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
