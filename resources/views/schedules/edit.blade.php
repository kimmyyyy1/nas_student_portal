                    ...
                    <form method="POST" action="{{ route('schedules.update', $schedule->id) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <select name="staff_id" id="staff_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    @foreach($staff as $person)
                                        <option value="{{ $person->id }}" {{ old('staff_id', $schedule->staff_id) == $person->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="text" name="day" id="day" placeholder="e.g., MWF or TTh" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ old('day', $schedule->day) }}" required>
                            </div>

                            {{-- Time Start --}}
                            <div>
                                <label for="time_start" class="block text-sm font-bold text-gray-700 mb-1">Time Start</label>
                                <input type="time" name="time_start" id="time_start" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                       value="{{ old('time_start', date('H:i', strtotime($schedule->time_start))) }}" required>
                            </div>

                            {{-- Time End --}}
                            <div>
                                <label for="time_end" class="block text-sm font-bold text-gray-700 mb-1">Time End</label>
                                <input type="time" name="time_end" id="time_end" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                       value="{{ old('time_end', date('H:i', strtotime($schedule->time_end))) }}" required>
                            </div>

                            {{-- Room --}}
                            <div class="md:col-span-3">
                                <label for="room" class="block text-sm font-bold text-gray-700 mb-1">Room (Optional)</label>
                                <input type="text" name="room" id="room" placeholder="e.g., Room 101" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ old('room', $schedule->room) }}">
                            </div>
