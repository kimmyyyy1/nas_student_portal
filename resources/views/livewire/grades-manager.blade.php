    ...
    @if($view == 'grid')
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
            @forelse($sections as $section)
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition duration-200 overflow-hidden">
                    <div class="p-6">
                        {{-- Header ng Card --}}
                        <div class="flex justify-between items-start mb-4">
                            <div class="p-3 bg-indigo-50 rounded-lg text-indigo-600">
                                <i class='bx bx-save text-xl'></i> {{-- Floppy Icon style --}}
                            </div>
                            @if($section->adviser)
                                <div class="text-xs font-bold text-indigo-600 uppercase tracking-wide bg-indigo-50 px-2 py-1 rounded">
                        ...
                        <div class="pt-4 border-t border-gray-100">
                            <button wire:click="openGradingSheet({{ $section->id }})" 
                                    class="w-full flex justify-between items-center text-sm font-bold text-indigo-600 hover:text-indigo-800 transition group">
                                OPEN GRADING SHEET
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($students as $student)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    {{-- Sample Input Fields for Grades --}}
