                ...
                <form id="applicantForm" method="POST" action="{{ route('applicant.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-8 sm:mb-10 bg-indigo-50 p-6 sm:p-8 rounded-xl border border-indigo-100 flex flex-col md:flex-row items-center gap-6 sm:gap-8">
                        <div class="flex-shrink-0">
                            <div class="w-40 h-40 sm:w-52 sm:h-52 bg-white border-4 border-dashed border-indigo-300 flex items-center justify-center text-indigo-400 rounded-lg overflow-hidden relative shadow-sm mx-auto">
                        ...
                        <div class="flex-1 w-full text-center md:text-left">
                            ...
                            class="block w-full text-sm text-slate-500 file:mr-4 file:py-3 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-indigo-600 file:text-white hover:file:bg-indigo-700 cursor-pointer border border-gray-300 rounded-md bg-white">
                        </div>
                    </div>

                    {{-- 1. APPLICANT INFO --}}
                    <div class="mb-8">
                        <h3 class="text-lg font-bold text-gray-800 border-b-2 border-gray-200 pb-2 mb-6 flex items-center">
                            <span class="bg-gray-800 text-white rounded-full h-6 w-6 flex items-center justify-center text-xs mr-2">1</span> Applicant Information
                        </h3>
                        <div class="grid grid-cols-1 gap-6 mb-6">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">LRN (12 Digits) *</label>
                                <input type="text" name="lrn" class="w-full rounded-lg border-gray-300 h-11 focus:border-indigo-500" required value="{{ old('lrn') }}" placeholder="12-digit LRN" maxlength="12" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 12)">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Last Name *</label><input type="text" name="last_name" class="w-full rounded-lg border-gray-300 h-11" required value="{{ old('last_name', Auth::user()->last_name ?? '') }}"></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">First Name *</label><input type="text" name="first_name" class="w-full rounded-lg border-gray-300 h-11" required value="{{ old('first_name', Auth::user()->first_name ?? '') }}"></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Middle Name</label><input type="text" name="middle_name" class="w-full rounded-lg border-gray-300 h-11" value="{{ old('middle_name', Auth::user()->middle_name ?? '') }}"></div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-6">
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Birthday *</label><input type="date" id="date_of_birth" name="date_of_birth" class="w-full rounded-lg border-gray-300 h-11" required value="{{ old('date_of_birth') }}" @change="calculateAge($event.target.value)"></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Age</label><input type="number" id="age" name="age" class="w-full rounded-lg border-gray-300 bg-gray-100 h-11 text-gray-600 font-bold" value="{{ old('age') }}" readonly x-model="age"></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Sex *</label><select name="gender" class="w-full rounded-lg border-gray-300 h-11" required><option value="">Select</option><option value="Male">Male</option><option value="Female">Female</option></select></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Place of Birth *</label><input type="text" name="birthplace" class="w-full rounded-lg border-gray-300 h-11" required value="{{ old('birthplace') }}"></div>
                        </div>
                         <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                             <div><label class="block text-sm font-bold text-gray-700 mb-2">Religion</label><input type="text" name="religion" class="w-full rounded-lg border-gray-300 h-11" value="{{ old('religion') }}"></div>
                             <div><label class="block text-sm font-bold text-gray-700 mb-2">Email Address</label><input type="email" name="email_address" class="w-full rounded-lg border-gray-300 bg-gray-100 h-11 text-gray-500" required value="{{ Auth::user()->email ?? '' }}" readonly></div>
                         </div>
                    </div>

                    {{-- 2. ADDRESS INFO --}}
                    <div class="mb-8">
                        <h3 class="text-lg font-bold text-gray-800 border-b-2 border-gray-200 pb-2 mb-6 flex items-center"><span class="bg-gray-800 text-white rounded-full h-6 w-6 flex items-center justify-center text-xs mr-2">2</span> Address Information</h3>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">Region *</label>
                                <select name="region" x-model="selectedRegion" @change="updateProvinces()" class="w-full rounded-lg border-gray-300 h-11" required>
                            ...
                            <div>
                                ...
                                <select name="province" x-model="selectedProvince" class="w-full rounded-lg border-gray-300 h-11" required :disabled="!selectedRegion">
                                    <option value="">Select Province</option>
                                    <template x-for="province in availableProvinces" :key="province"><option :value="province" x-text="province"></option></template>
                                </select>
                            </div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Municipality/City *</label><input type="text" name="municipality_city" class="w-full rounded-lg border-gray-300 h-11" required value="{{ old('municipality_city') }}"></div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Barangay *</label><input type="text" name="barangay" class="w-full rounded-lg border-gray-300 h-11" required value="{{ old('barangay') }}"></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Street / House No.</label><input type="text" name="street_address" class="w-full rounded-lg border-gray-300 h-11" value="{{ old('street_address') }}" required></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Zip Code</label><input type="text" name="zip_code" class="w-full rounded-lg border-gray-300 h-11" value="{{ old('zip_code') }}" required maxlength="4" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 4)"></div>
                        </div>
                    </div>

                    {{-- 3. ACADEMIC & SPORTS --}}
                    <div class="mb-8">
                        <h3 class="text-lg font-bold text-gray-800 border-b-2 border-gray-200 pb-2 mb-6 flex items-center"><span class="bg-gray-800 text-white rounded-full h-6 w-6 flex items-center justify-center text-xs mr-2">3</span> Academic & Sports</h3>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Last School Attended *</label><input type="text" name="previous_school" class="w-full rounded-lg border-gray-300 h-11" required value="{{ old('previous_school') }}"></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">School Type *</label><select name="school_type" class="w-full rounded-lg border-gray-300 h-11"><option value="Public">Public</option><option value="Private">Private</option></select></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Grade Level Applying For *</label><select name="grade_level_applied" class="w-full rounded-lg border-gray-300 h-11" required><option value="">Select</option><option value="Grade 7">Grade 7</option><option value="Grade 8">Grade 8</option></select></div>
                        </div>
                        ...
                        <div class="bg-indigo-50 p-6 rounded-lg mb-6 border border-indigo-100">
                            ...
                            <select name="sport" x-model="selectedSport" class="w-full rounded-lg border-gray-300 h-11 focus:border-indigo-500 mb-4" required>
                            ...
                            <div x-show="selectedSport === 'Aquatics'" class="mt-2"><label class="block text-xs font-bold text-gray-600 mb-1">Please specify Aquatics event:</label><input type="text" name="sport_specification" class="w-full rounded-md border-gray-300 h-10 text-sm"></div>
                            <div x-show="selectedSport === 'Athletics'" class="mt-2"><label class="block text-xs font-bold text-gray-600 mb-1">Please specify Athletics event:</label><input type="text" name="sport_specification" class="w-full rounded-md border-gray-300 h-10 text-sm"></div>
                            <div x-show="selectedSport === 'Taekwondo'" class="mt-2"><label class="block text-xs font-bold text-gray-600 mb-1">Category:</label><select name="sport_specification" class="w-full rounded-md border-gray-300 h-10 text-sm"><option value="Poomsae">Poomsae</option><option value="Kyorugi">Kyorugi</option></select></div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg border">
                                ...
                                <div class="flex space-x-4">
                                    <label class="flex items-center space-x-2"><input type="radio" name="batang_pinoy_finisher" value="Yes" class="form-radio text-indigo-600 w-4 h-4"><span class="text-sm font-medium text-gray-700">Yes</span></label>
                                    <label class="flex items-center space-x-2"><input type="radio" name="batang_pinoy_finisher" value="No" class="form-radio text-indigo-600 w-4 h-4" checked><span class="text-sm font-medium text-gray-700">No</span></label>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- 4. BACKGROUND INFO --}}
                    <div class="mb-8">
                        <h3 class="text-lg font-bold text-gray-800 border-b-2 border-gray-200 pb-2 mb-6 flex items-center"><span class="bg-gray-800 text-white rounded-full h-6 w-6 flex items-center justify-center text-xs mr-2">4</span> Background Information</h3>
                        <div class="grid grid-cols-1 gap-6 mb-6">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">Where did you learn about NAS?</label>
                                <select x-model="referralSource" name="learn_about_nas" class="w-full rounded-lg border-gray-300 h-11">
                            ...
                            <div x-show="referralSource === 'NAS Personnel / Student-Athlete Referral'" class="bg-yellow-50 p-4 rounded-md border border-yellow-200"><label class="block text-sm font-bold text-yellow-800 mb-1">If referred, write the name (One name only):</label><input type="text" name="referrer_name" class="w-full rounded-md border-gray-300 h-10"></div>
                            <div>
                        ...
                        <div class="bg-gray-50 p-6 rounded-xl border border-gray-200 space-y-4">
                            ...
                            <div>
                                ...
                                <div class="flex space-x-4"><label class="flex items-center"><input type="radio" name="is_4ps" value="Yes" class="mr-2 text-indigo-600"> Yes</label><label class="flex items-center"><input type="radio" name="is_4ps" value="No" class="mr-2 text-indigo-600" checked> No</label></div>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- 5. GUARDIAN --}}
                    <div class="mb-8">
                        <h3 class="text-lg font-bold text-gray-800 border-b-2 border-gray-200 pb-2 mb-6 flex items-center"><span class="bg-gray-800 text-white rounded-full h-6 w-6 flex items-center justify-center text-xs mr-2">5</span> Designated Guardian</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">Guardian Name *</label><input type="text" name="guardian_name" class="w-full rounded-lg border-gray-300 h-11" required value="{{ old('guardian_name') }}"></div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach([
                                ...
                                <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col hover:shadow-md transition">
                                    ...
                                    <input type="file" name="files[{{ $key }}]" class="block w-full text-xs text-slate-600 file:mr-4 file:py-2.5 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-indigo-600 file:text-white hover:file:bg-indigo-700 cursor-pointer" accept=".pdf,.jpg,.jpeg,.png" required>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="flex justify-center pb-8 pt-4">
                        <button type="button" @click="if(document.getElementById('applicantForm').reportValidity()) { showPrivacyModal = true }" class="w-full sm:w-auto bg-indigo-700 hover:bg-indigo-800 text-white px-10 py-3 rounded-lg font-bold text-lg shadow-md transition transform hover:scale-105">REVIEW & SUBMIT APPLICATION</button>
                    </div>
                </form>
            </div>
        </div>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div class="mt-2 h-48 overflow-y-auto text-sm text-gray-600 bg-gray-50 p-4 rounded border border-gray-200 text-justify">
                                    ...
                                    <ul class="list-disc ml-5 mb-3 space-y-2"><li>Collection...</li><li>Use...</li><li>Protection...</li></ul>
