{{-- CSS OPTIMIZATION --}}
<style>
    /* Pinipigilan ang default smooth scroll para instant ang lipat ng position (iwas flicker) */
    .sidebar-scroll-area { scroll-behavior: auto !important; }
...
<div x-data="{ open: false }">
    ...
    {{-- 3. SIDEBAR (Ibinalik sa dating layout na gusto mo) --}}
    <nav :class="{'translate-x-0': open, '-translate-x-full': !open}"
        ...
        {{-- 
            👇 SCROLLABLE AREA (ANTI-FLICKER FIX)
            Gamit ang x-data at x-init dito mismo sa div para mabilis ang restore.
        --}}
        <div class="flex-1 overflow-y-auto no-scrollbar sidebar-scroll-area flex flex-col"
             x-data="{
                 init() {
                     let savedPos = sessionStorage.getItem('sidebarScrollPos');
                     if (savedPos) {
                         this.$el.scrollTop = savedPos;
                     }
                 },
                 saveScroll() {
                     sessionStorage.setItem('sidebarScrollPos', this.$el.scrollTop);
                 }
             }"
             x-init="init()"
